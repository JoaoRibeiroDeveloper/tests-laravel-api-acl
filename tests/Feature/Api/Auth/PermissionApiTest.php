...
test('should return permission', function () {
    getJson(
        route('permissions.show', $this->permission->id),
        ['Authorization' => "Bearer {$this->token}"],
    )
    ->assertOk()
    ->assertJsonStructure([
...
test('should return 404 when permission not found', function () {
    getJson(
        route('permissions.show', 'fake_id'),
        ['Authorization' => "Bearer {$this->token}"],
    )
    ->assertNotFound();
});
...
test('should delete permission', function () {
    ...
    assertDatabaseMissing('permissions', ['id' => $this->permission->id]);
});

test('should return 404 when not exists permission - delete', function () {
    deleteJson(
        route('permissions.destroy', 'fake_id'),
        [],
        ['Authorization' => "Bearer {$this->token}"],
    )->assertNotFound();
});
